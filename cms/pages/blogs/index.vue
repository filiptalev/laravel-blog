<template>
  <div id="sc-page-wrapper">
          ...
          >
            <template slot="table-row" slot-scope="props">
              <span v-if="props.column.field == 'actions'">
                <button
                  class="uk-button uk-button-primary"
                  @click="props.row.role == edit(props.row.id)"
                >
                  Edit
                </button>
                <button
    ...
    <div id="modal-center" class="uk-flex-top uk-modal" data-uk-modal>
      <div
        class="uk-modal-dialog uk-modal-body uk-margin-auto-vertical text-center"
        style="text-align: center"
      >
        <h1>Are you sure?</h1>
        <div>
          <button class="uk-button uk-button-danger uk-modal-close" @click="deleteItem">
            Delete
          </button>
    ...
    <div id="modal-activate" class="uk-flex-top uk-modal" data-uk-modal>
      ...
      >
        <h1>Are you sure?</h1>
        <div>
          <button class="uk-button black-btn uk-modal-close" @click="activateUser">
            Activate
          </button>
          <button class="uk-button uk-button-primary uk-modal-close">Cancel</button>
        </div>
...
export default {
  ...
  methods: {
    async deleteItem() {
      try {
        await this.$axios.post(`blogs/${this.rowId}`, { _method: "delete" });
        this.loadItems();
      } catch (e) {
    ...
    async loadItems() {
      this.loading = true;
      await this.$axios.post(`/blogs-paginated`, this.serverParams).then((response) => {
