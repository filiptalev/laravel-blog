  ...
  >
    <li
      ...
      :class="{
        ...
        'sc-page-active': $route.path.includes(item.page) || pageChild(item),
    ...
    >
      ...
      <a
        ...
        @click.stop="toggleSection($event, item)"
      >
        <span v-if="item.icon" class="uk-nav-icon">
          <i :class="item.icon"></i>
        </span>
...
export default {
  ...
  computed: {
    ...
    menuItems() {
      ...
      var newMenuData = this.menuData.filter(function (menuItem) {
        if (menuItem.roles && menuItem.roles.length > 0) {
          var itemFound = menuItem.roles.find(
            (role) => role === self.$auth.user.role
          );
          if (!!itemFound) return menuItem;
        }
      });
      return newMenuData;
    },
  ...
  methods: {
    pageChild(item) {
      return this.$nuxt.$route.matched.some(
        (route) => route.path === item.page
      );
    },
